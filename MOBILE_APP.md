...
Photo-free by design: the Send invites, Invitations and Considerations tiles (they hand off to desktop), the Emergency contact tile (a form) and the guest suite Accommodation tile. The demo airport (`placeAirport`) was the one slot without a photo; the owner added one after goal 5 (`hf_20260921_234435_..._nxxjhz`, a couple walking toward the gates with suitcases, at the cloud's root rather than in `app/`), so the manifest now holds 73 slots and 73 distinct photos. **Still open: 1.** The guest suite Accommodation tile stays an icon tile; the one unassigned library photo (a couple in bed, `DTS_SUITE_TALK ... 14160`) has closed eyes in its focal area and stays out, as goal 4 ruled, so a photo for that tile is a second one to add.
