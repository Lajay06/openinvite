...
| Piece | Built | Notes |
|---|---|---|
| Icon and splash | `assets/logo.png`, `assets/icon-only.png`, `assets/icon-foreground.png`, `assets/icon-background.png`, `assets/splash.svg`, `assets/splash-dark.svg`, generated into both projects with `npx capacitor-assets generate` | Goal 6: the brand mark (the arch) on ink, see the icon-source note below; the splash is the mark centered on the flat page background, dark variant on ink, no gradient. The splash fades out over 250ms once the shell has painted (`hideSplash()` 350ms after mount). The wordmark PNG in the repo is 1434 by 331 and unsuitable for a square icon, so the mark was drawn from the favicon rather than rasterized from it. |
...
- **Icon source** (goal 6): the app icon is the Openinvite brand mark, the arch with the ribbon, cropped from `public/openinvite-logo.png` (the only copy in the repo; the mark is 284 by 282 there and in the hosted email asset) and resampled onto ink at 60 percent of a 1024 canvas in `assets/icon-only.png`, with `assets/icon-foreground.png` and `assets/icon-background.png` for the Android adaptive icon. Not redrawn. A vector or a larger raster of the mark would sharpen the App Store 1024 icon; drop it at `assets/icon-only.png` and re-run `npx capacitor-assets generate --ios --android` (then discard the splash drawables it writes; the launch screen is hand-drawn, see goal 3). The native launch logo (`ios/App/App/launch-logo*.png`, `android/.../launch_logo.png`) is the mark alone; the in-app splash keeps the full wordmark.
...
Photo-free by design: the Send invites, Invitations and Considerations tiles (they hand off to desktop) and the Emergency contact tile (a form). **The photo to-do list is empty.** The owner added two photos after goal 5, both at the cloud's root rather than in `app/`: `hf_20260921_234435_..._nxxjhz` (a couple walking toward the gates with suitcases) for the demo airport, and `DTS_Remote_Studio_Tino_Renato_Photos_ID3731_bqr4fe` (reading on a poolside lounger) for the guest suite Accommodation tile. The manifest now holds 74 slots and 74 distinct photos; the one library photo still unassigned (a couple in bed, `DTS_SUITE_TALK ... 14160`) has closed eyes in its focal area and stays out, as goal 4 ruled.
...
Built against `MOBILE_APP_GOAL_6.md`, one commit per item (items 2 and 4 share one, because the launch timing is defined by the daily update stage). Everything the goal asks for is in; the splash pool stands at 14 of the 20 asked for, see "Photos" below.
...
9. **Photo fixes.** The Styling tile has a different photo (a silver skirt and a cocktail, an outfit with no face to cut); the earlier one was framed below the chin, so every crop cut the face, and it stays unassigned. The Emergency contact tile (head down over a laptop in a booth, cropped from the top through the per-slot `gravity` that `deliver()` honors) and the Considerations tile (a glass of water with a sprig of flowers) have photos from the owner's 2026-09-22 batch, used nowhere else. The rendered-photo audit was re-run over all 74 preview routes: nothing renders that is not a slot, and the only repeats are the two documented ones (the couple's cover as their identity, the Beach Hotel as one place in two lists).
...
Goal 6 asks for a splash pool of 20 to 30 unused photos, two more tiles and a demo couple pool. At the first pass the `app/` library was spent (goal 5 had assigned every usable photo), so the pool held 2 and the two tiles waited as color panels. The owner then added 22 photos to `app/` on 2026-09-22 (95 in the folder). Every one was viewed as a thumbnail, and each pool candidate again at the phone's crop behind the logo and scrim, before assignment. Of the 22: one went to the Styling tile (replacing the headless photo), one each to the Emergency contact and Considerations tiles, 12 to the splash pool (`splash3` to `splash14`), and 7 were left out against the goal's own test for pool photos, calm, strong, faces not cut off, no closed eyes: four with closed eyes in the focal area (the forehead-to-forehead pair, the kiss under the lamp, the bar in black and white, the back seat), one motion-blurred leap in a pink dress, one with curlers in bed on a phone, and one with the face cut at the nose over a champagne glass. The demo couple pool is the five fixture stand-ins already reserved for the couple's own photos. `/m/preview/images` reads 88 slots, 88 photos, no repeats, none needing a photo. **The splash pool holds 14. Photos still needed: 6 to reach the goal's minimum of 20, 16 for a 30-photo pool.** Unused photos in `app/`: 9 (the 7 above, the couple in bed that goal 5 left out for closed eyes, and the Styling tile's former photo, framed below the chin), none of which meets the pool test.
...
### What else changed (goal 8)

- **The daily update is a full-screen takeover** (item 1): edge to edge, the photo in the top 45 percent under the status bar with a light wash for the clock, the red panel below with a 28px rounded top edge; fade in, fade out over the dashboard, swipe down still closes. All seven bundled photos are people facing the camera (see Photos below).
- **The top bar on every tab root** (item 2): the arch mark at 28px top left (Home from any other tab), then search, messages with its own unread count, notifications with the dot, top right; the row stays as the bar collapses. `Screen`'s `bell` prop became `root`. The mark is the app icon's own artwork (`src/mobile/assets/mark.png`, from `assets/icon-foreground.png`), the one gradient in the app because it is the brand mark, not a panel.
- **Guest numbers** (item 3), **the guest suite note** (item 4), **the previews through the desktop renderers** (item 5), **Send invites** (item 6), **forecasting and Ava** (item 7), **contacts** (item 9), **calendar** (item 10) and **the offline day** (item 11) are each recorded under their feature in `MOBILE_PARITY.md`; the real login track (item 8) and the calendar feed finding are above.

### Photos (goal 8)

The daily set was re-cast to people facing the camera, faces whole, eyes open, warm and candid, no backs turned. The unused `app/` pool held sixteen photos and one passed that test (the two in helmets through a car window, `hf_20260903_230909..._hqk13n`); the others are profiles, closed eyes, backs, a face cut at the eyes, a motion blur, a bouquet and a martini. So six daily photos came from slots that held face-to-camera photos (the two friends from the Photography tile, the sofa couple from Messages, the photo booth from the demo's first marketplace result, the blanket couple from keep-planning card 1, the party couple from Polls, the couple at dusk from the guest suite Schedule tile) and those six slots took the best of the unused pool (the kiss in the tall window, the couple laughing in the blue doorway, the flower market, the bouquet on the move, the leap from the rocks, the curlers in a hotel bed). Exported at 800 by 800 (`c_fill,g_auto`) into `src/mobile/assets/daily/`. `/m/preview/images` reads 81 slots, 81 photos, no repeats. Unused photos in `app/` are now 10; none passes the daily test.
