...
**Status.** Planned (phases 1 and 2): detail sheet with title, description, priority, due date, status, delete; filter and sort; CSV export; the completion animation, Undo toast, collapsible Completed section per group, untick from Completed.
...
**Status.** Planned (phases 1 and 2): the profile shows reply per event with meal, plus-one and responded date, contact rows, group, tags, table, meal, dietary, plus-one details, address, notes, invitation history (sent, channel, reminder), RSVP note and song request, gifts from this guest; edit sheet with every desktop field; Edit events sheet; per-guest copy link and send; import from a CSV or XLSX file; export CSV; select mode with the bulk bar (category, dietary, add tag, remove tag, set events, copy links, send, delete).
...
**Status.** Planned (phases 1 and 2): Polls segment (templates or custom, title, options, comments toggle; card with counts, insight, comments; edit sheet for title, options and comments; end, reopen, delete, share) and Games segment (list, new game with questions and recipients, results per question, copy links, close or reopen, delete).
...
**Status.** Planned (phase 1): search, the count line, mark unread from the thread, WhatsApp compose sheet with the same templates (opens WhatsApp natively), the couple's number under a settings sheet (stored on the device as the desktop does).
...
**Gaps.** The whole flow.
**Status.** Planned (phase 1): a four-step full-screen flow with the same types, filters, search, compose defaults, merge tags, banner choice, test send, channel, review and send, and the same tracking writes. The rendered email preview is shown in a sheet.
...
**Status.** Planned (phase 1): Requests segment with the five filters and approve, add, decline; Playlist segment with the editable link and the source label; Settings sheet; Share sheet (link, native share, QR); Notes; Vendors row into My vendors filtered to music. The Music entity playlist editor is kept.
...
**Status.** Planned (phase 1): search, status and category filters, favourite toggle, the full form (photo and video fields appear for those categories), a vendor detail screen with logs, documents (upload) and tasks.
...
**Status.** Planned (phase 1): Plan sheet (total and per-category allocation), delete from the expense sheet, search, Forecast segment with the recommended split and Ava insights, both exports.
