...
- **`src/pages/`** (97 files) — mostly one file per route, both dashboard pages (Guests, Budget, Vendors, EventDetails, AvaStudio, UniverseStudio, StudioHub, StudioGuestSuite, Account, …) and the small number of dedicated guest-only routes (`GuestAccommodation.jsx`, `GuestMusic.jsx`, `GuestCollect.jsx`). A file here is **not** automatically a live route, and the route table alone will not settle it either — routes come from *two* sources: the explicit `lazyWithReload` imports in `App.jsx`, and the auto-generated registry in `pages.config.js`. Three cases to know before trusting a page is live *or* dead:
  - **Unrouted but live.** `Calendar.jsx`, `Schedule.jsx`, `Checklist.jsx` and `TodoList.jsx` appear in neither source, yet all ship — they are imported as tab content by the routed `ScheduleHub.jsx` and `TasksHub.jsx`.
  - **Unrouted, unimported, still reachable.** `OAuthConsent.jsx` is imported by nothing in `src/`, but the Base44 platform redirects to it via the `consent_path` in its MCP config — which lives server-side, not in this repo.
  - **Genuinely dead.** These do get deleted rather than left lying around: `src/lib/PageNotFound.jsx` in #414 and `src/pages/WeddingWebsite.jsx` in #437.

  So: trace the imports, and check both route sources plus any platform-side entry point. File existence proves nothing in either direction.
