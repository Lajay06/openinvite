...
const SHOTS = [
  ...
  ['/m/preview/search?q=a', 'search-results'],
  ...
  ['/m/preview/priming?test=1', 'notification-priming-test'],
...
for (const [route, name, action] of SHOTS.filter(([, n]) => !ONLY || ONLY.has(n))) {
  ...
  const ok = (m.chars > 100 || name.includes('state-loading')) && m.wide === 0 && m.small === 0 && m.inputs === 0 && m.shadows === 0;
  if (!ok) failures++;
