        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          ...
          {tab === 'website' && (
            <div>
              ...
              <div style={{ padding: 16, background: '#F8F8F8', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontWeight: 600, fontSize: 14 }}>{details?.websiteEnabled ? 'Your website is live' : 'Your website is not published yet'}</p>
                  <p style={{ margin: 0, fontSize: 12, color: '#888' }}>{siteUrl}</p>
                </div>
                <button
                  onClick={togglePublish}
                  style={{
                    padding: '8px 20px', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'inherit',
              ...
              <div style={{ display: 'flex', alignItems: 'stretch', marginBottom: 20, borderBottom: '1px solid #DDD' }}>
                ...
                />
                <button onClick={saveSlug} style={{ padding: '10px 16px', background: '#0A0A0A', color: '#FFF', border: 'none', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Save</button>
              </div>
          ...
          {tab === 'share' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  { label: 'WhatsApp', icon: '💬', action: () => window.open(`https://wa.me/?text=You're invited! ${siteUrl}`) },
                  { label: 'Email', icon: '✉️', action: () => setTab('email') },
                  { label: 'SMS', icon: '📱', action: () => window.open(`sms:?body=You're invited! ${siteUrl}`) },
                  { label: 'Facebook', icon: '📘', action: () => window.open(`https://www.facebook.com/sharer/sharer.php?u=https://${siteUrl}`) },
                ].map(opt => (
          ...
          {tab === 'email' && (
            <div>
              ...
              <input
                defaultValue={`${couple1} & ${couple2}'s Wedding — Save the Date`}
              ...
              <textarea
                rows={6}
                defaultValue={`We're so excited to share our wedding website with you!\n\nVisit: https://${siteUrl}\n\nWe can't wait to celebrate with you.\n\nWith love,\n${couple1} & ${couple2}`}
              ...
              <button style={{ width: '100%', padding: '14px', background: 'linear-gradient(135deg, #E03553, #803D81)', color: '#FFF', border: 'none', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
