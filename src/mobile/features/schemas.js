...
export const DETAILS = {
  styling: {
    ...
    sections: [
      ...
      { title: 'Outfits', segment: 'attire', key: 'attire', list: { name: 'outfits', label: 'Outfit', id: uid, image: 'photoUrl', required: ['role'], row: (o) => ({ title: o.name ? `${o.name}, ${o.role === 'Other' ? (o.roleCustom || 'Other') : (o.role || 'Outfit')}` : (o.role === 'Other' ? (o.roleCustom || 'Other') : (o.role || 'Outfit')), sub: [o.status, o.source].filter(Boolean).join(', '), image: o.photoUrl }), fields: [
      ...
      { title: 'Accessories', segment: 'attire', key: 'attire', list: { name: 'accessories', label: 'Accessory', plural: 'accessories', id: uid, required: ['item'], row: (a) => ({ title: a.item || 'Accessory', sub: [a.for, a.notes].filter(Boolean).join(', ') }), fields: [t('item', 'Item', { placeholder: 'Veil, cufflinks, garter' }), t('for', 'For whom'), ta('notes', 'Notes')] } },
  ...
  favours: {
    ...
    sections: [
      { title: 'The gift', segment: 'overview', key: 'weddingFavours', fields: [t('concept', 'What you are giving', { type: 'search', placeholder: 'Local honey, a candle, a seedling' }), t('supplierName', 'Supplier or maker', { type: 'search' }), t('totalBudget', 'Total budget', { type: 'number' }), sel('orderedStatus', 'Status', [['not_started', 'Not started'], ['researching', 'Researching'], ['ordered', 'Ordered'], ['received', 'Received'], ['assembled', 'Assembled and ready']])] },
      { title: 'Favour items', segment: 'items', key: 'weddingFavours', list: { name: 'favourItems', label: 'Item', row: (i) => ({ title: i.name || 'Item', sub: [i.quantity ? `${i.quantity} of them` : '', i.notes].filter(Boolean).join(', '), value: i.costPerUnit ? `${i.costPerUnit} each` : '' }), fields: [t('name', 'Item'), t('quantity', 'Quantity', { type: 'number' }), t('costPerUnit', 'Cost per unit', { type: 'number' }), ta('notes', 'Notes')] } },
  ...
  ceremony: {
    ...
    sections: [
      { title: 'Celebrant', segment: 'celebrant', key: 'celebrant', encrypted: true, fields: [t('name', 'Celebrant name', { type: 'search' }), t('title', 'Title'), sel('type', 'Type', [['celebrant', 'Professional celebrant'], ['religious', 'Religious officiant'], ['friend', 'Friend or family'], ['registry', 'Registry office']]), t('phone', 'Phone', { type: 'tel' }), t('email', 'Email', { type: 'email' }), ta('notes', 'Celebrant notes')] },
  ...
  transport: {
    ...
    sections: [
      ...
      { title: 'Couple-arranged transport', segment: 'shuttles', key: 'transport', list: { name: 'shuttles', label: 'Shuttle', id: () => `s_${Date.now()}`, required: ['name'], row: (s) => ({ title: s.name || 'Shuttle', sub: [s.pickupLocation && s.pickupTime ? `${s.pickupLocation} at ${s.pickupTime}` : s.pickupLocation, s.dropoffLocation].filter(Boolean).join(' to '), value: s.capacity ? `${s.capacity} seats` : '' }), fields: [t('name', 'Service name'), sel('type', 'Type', [['coach', 'Coach'], ['shuttle', 'Shuttle bus'], ['minibus', 'Minibus'], ['transfer', 'Transfer'], ['limo', 'Limousine']]), t('pickupLocation', 'Picks up from'), t('pickupTime', 'Pick-up time', { type: 'time' }), t('dropoffLocation', 'Drops off at'), t('returnTime', 'Return time', { type: 'time' }), t('capacity', 'Seats', { type: 'number' }), t('contact', 'Contact'), ta('notes', 'Notes')], defaults: { type: 'coach' } } },
