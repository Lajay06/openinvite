...
export const ENTITIES = {
  schedule: {
    ...
    sort: 'start_time',
    itemLabel: 'event',
    row: (r) => ({ title: r.event_name, sub: [r.event_date, r.start_time].filter(Boolean).join(' at '), value: r.location }),
  ...
  vows: {
    ...
    sort: '-created_date',
    itemLabel: 'draft',
    row: (r) => ({ title: r.title || (r.type === 'vow' ? 'Vows' : 'Speech'), sub: [r.type === 'vow' ? 'Vows' : 'Speech', r.author].filter(Boolean).join(' · ') }),
  ...
  vendors: {
    ...
    sort: '-created_date',
    itemLabel: 'vendor',
    row: (r) => ({ title: r.name, sub: [r.category, r.quoted_price ? `$${Number(r.quoted_price).toLocaleString('en-US')}` : ''].filter(Boolean).join(' · '), badge: VENDOR_STATUS_LABEL[r.status] || r.status, badgeTone: r.status === 'booked' ? 'ok' : r.status === 'rejected' ? 'no' : r.status === 'researching' ? 'neutral' : 'warn' }),
  ...
  'registry-links': {
    ...
    sort: '-created_date',
    itemLabel: 'registry',
    row: (r) => ({ title: r.store_name || 'Registry', sub: r.url, image: r.image_url }),
  ...
  'registry-products': {
    ...
    sort: '-created_date',
    itemLabel: 'product',
    row: (r) => ({ title: r.name, sub: [r.price ? `$${Number(r.price).toLocaleString('en-US')}` : '', r.quantity_purchased ? `${r.quantity_purchased} of ${r.quantity_requested || 1} bought` : ''].filter(Boolean).join(' · '), image: r.image_url }),
  ...
  'registry-funds': {
    ...
    sort: '-created_date',
    itemLabel: 'fund',
    row: (r) => ({ title: r.title, sub: r.requested_amount ? `Goal $${Number(r.requested_amount).toLocaleString('en-US')}` : r.description, image: r.image_url }),
  ...
  'registry-received': {
    ...
    sort: '-created_date',
    itemLabel: 'gift',
    row: (r) => ({ title: r.item_name || 'Gift', sub: [r.giver_name, r.estimated_value ? `$${Number(r.estimated_value).toLocaleString('en-US')}` : ''].filter(Boolean).join(' · '), badge: r.thank_you_sent ? 'Thanked' : 'Thank you to send', badgeTone: r.thank_you_sent ? 'ok' : 'warn' }),
  ...
  music: {
    ...
    sort: '-created_date',
    itemLabel: 'track',
    row: (r) => ({ title: r.song_title || 'Track', sub: [r.artist, r.category].filter(Boolean).join(' · '), image: r.image_url }),
