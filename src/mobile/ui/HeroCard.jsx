 ...
 * ink panel, so nothing is ever blank.
 */
export default function HeroCard({ image, alt = '', label, title, number, sub, action, onAction, short = false, topRight, children, onClick }) {
  ...
  return (
    <Tag type={onClick ? 'button' : undefined} className={`oi-m-hero${short ? ' oi-m-hero--short' : ''}${onClick ? ' oi-m-press' : ''}`} onClick={onClick} ref={ref}>
      <SmartImage src={image} alt={alt} width={360} ratio={short ? '4/3' : '4/5'} square eager tone="ink" style={{ transform: `translateY(${y}px) scale(1.06)`, transition: 'transform 80ms linear' }} />
