    ...
    if (step.aiGenerate) {
      ...
      try {
        const story = await base44.integrations.Core.InvokeLLM({
          prompt: `Write a warm, personal 3-paragraph love story for a couple's guest suite. Keep it genuine and heartfelt, not cheesy. Couple: ${coupleName}. Wedding date: ${details?.weddingDate || 'upcoming'}. Wedding style: ${JSON.stringify(details?.weddingStyle || [])}. Any notes: ${details?.coupleStory || 'none provided'}. Write in first person plural ("We..."). Keep each paragraph 2-3 sentences. End on the wedding. Output only the story text, no headings.`,
        });
