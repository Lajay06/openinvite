...
export default function TodoList({ embedded = false }) {
  ...
  // THE CSV EXPORT IS GONE — owner ruling, round two item 6: "To do: remove
  // the Export CSV button." exportNotes went with it rather than being left
  // unreferenced.
  //
  // What it carried is not lost: a to-do with a date is already a row on the
  // schedule, and the schedule's export is where a list of them leaves the
  // product. The reasoning for its columns — that the kanban `status` is the
  // one fact a flat list drops, so Ideas / In progress / Done do not collapse
  // into each other — is kept in tests/persistence/notes-export.mjs, where a
  // future export can start from a complete answer rather than a fresh guess.
